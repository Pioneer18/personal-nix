...
const RALPH_SKILLS_DIR = join(HOME, ".claude", "skills", "ralph");
...
function readRalphState(wtPath: string): Pick<RalphState, "status" | "pid" | "iter" | "lastProgress"> {
  const r = join(wtPath, ".ralph");
  ...
  let status: RalphState["status"] = "unknown";
  ...
  if (existsSync(pidFile)) {
    ...
    if (!isNaN(p) && pidAlive(p)) {
      ...
      status = existsSync(outcomeFile)
        ? (readFileSync(outcomeFile, "utf8").trim() as RalphState["status"])
        : "unknown";
    }
  } else if (existsSync(outcomeFile)) {
    status = readFileSync(outcomeFile, "utf8").trim() as RalphState["status"];
  }
...
function ralphStatus(): RalphState[] {
  ...
  return readdirSync(PROJECTS_DIR, { withFileTypes: true })
    ...
    .flatMap(entry => {
      ...
      return parseWorktrees(porcelain)
        .filter(wt => wt.branch.startsWith("ralph/") && existsSync(join(wt.path, ".ralph")))
        .map(wt => ({
          repo: entry.name,
          branch: wt.branch,
          worktree: wt.path,
          ...readRalphState(wt.path),
        }));
    });
}
...
async function ralphDispatch(cap: number): Promise<object> {
  ...
  const branch = `ralph/${next.slug}`;
  const worktreePath = join(dirname(next.targetRepo), `${repoName}-ralph-${next.slug}`);
  ...
  writeFileSync(join(ralphDir, "base_branch"), baseBranch);

  // .gitignore
  const gitignorePath = join(worktreePath, ".gitignore");
  const existing = existsSync(gitignorePath) ? readFileSync(gitignorePath, "utf8") : "";
  if (!existing.includes(".ralph/")) writeFileSync(gitignorePath, existing.trimEnd() + "\n.ralph/\n");

  // Render templates
  const feedback = detectFeedbackLoops(next.targetRepo);
  const allowedTools = buildAllowedTools(feedback);

  const ralphShPath = join(ralphDir, "ralph.sh");
  writeFileSync(ralphShPath, renderTemplate(
    readFileSync(join(RALPH_SKILLS_DIR, "ralph.sh.tmpl"), "utf8"),
    { REPO_PATH: worktreePath, SENTINEL, ALLOWED_TOOLS: allowedTools }
  ));
  chmodSync(ralphShPath, "755");

  writeFileSync(join(ralphDir, "prompt.md"), renderTemplate(
    readFileSync(join(RALPH_SKILLS_DIR, "prompt.md.tmpl"), "utf8"),
    {
      GOAL: next.goal,
  ...
  execSync(`git -C "${worktreePath}" commit -m "chore: scaffold ralph loop for ${next.slug}"`);
  ...
  const child = spawn(ralphShPath, ["--afk", String(cap)], {
    detached: true,
    stdio: "ignore",
    cwd: worktreePath,
  });
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: "ralph_status",
      description: "List all Ralph worktrees across ~/projects/ with status, PID liveness, iteration progress, and last progress note.",
      inputSchema: { type: "object" as const, properties: {} },
    },
    {
      name: "ralph_dispatch",
      description: "Grab the next open + ready work-request from ~/projects/personal-nix/wiki/work-requests/, scaffold a ralph worktree, and launch the AFK loop. Returns PID, branch, and worktree path.",
      inputSchema: {
